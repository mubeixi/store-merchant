    ...
    <div class="list style2" v-if="coupon.config.type===2"  >
      <div class="item" v-for="(item,idx) in coupon.value.list">
        <div class="l">
          ...
          <p class="title line4 font24" v-else><span class="font12 ">￥</span><span class="money">{{item.Coupon_Cash}}</span></p>

          <p class="info line"> {{item.Coupon_UseArea==0?'实体店':'微商城'}}可用 | 满￥{{item.Coupon_Condition}}可用</p>
