<template>
  <div class="addProduct">
    ...
    <el-form size="small" :model="ruleForm" :rules="rules" ref="ruleForm" :inline-message="true"
      ...
      <el-form-item label="商品排序" prop="Products_Index">
        <el-input v-model="ruleForm.Products_Index" :disabled="noEditField.Products_Index"
                  class="sortInput"></el-input>
        <span class="sortMsg">注：数字越大，越往前（必须大于0）</span>
      </el-form-item>
      <el-form-item label="商品名称" prop="Products_Name">
        <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                    :disabled="!noEditField.Products_Name">
          <el-input v-model="ruleForm.Products_Name" :disabled="noEditField.Products_Name"
      ...
      <div class="group cate_list" style="margin-left: 120px;margin-bottom: 22px;"
           v-if="cate_list.length>0">
        <span class="cate_item" v-for="(cate,idx) in show_cate_list">{{cate.Category_Name}}</span>
      </div>
      <el-form-item label="销量">
        <el-input v-model.number="ruleForm.Products_Sales" :disabled="noEditField.Products_Sales"
                  class="sortInput"></el-input>
      ...
      <el-form-item label="产品价格">
        <el-form-item prop="Products_PriceY" style="display: inline-block;margin-bottom: 0px"
                      v-if="need_price_y==1">
          <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                      :disabled="!noEditField.Products_PriceY">
            <el-input v-model="ruleForm.Products_PriceY" :disabled="noEditField.Products_PriceY"
                      placeholder="原价:¥" class="sortInput"></el-input>
          </el-tooltip>
        </el-form-item>
        <el-form-item prop="Products_PriceX" style="display: inline-block;margin-bottom: 0px">
          <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                      :disabled="!noEditField.Products_PriceX">
            <el-input v-model="ruleForm.Products_PriceX" :disabled="noEditField.Products_PriceX"
                      placeholder="现价:¥" class="sortInput" style="margin-left: 18px"></el-input>
          </el-tooltip>
        </el-form-item>
      </el-form-item>
      ...
      <el-form-item label="拼团" prop="type" style="margin-bottom: 10px">
        <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                    :disabled="!noEditField.pintuan_flag">
          <el-checkbox v-model="ruleForm.pintuan_flag" :disabled="noEditField.pintuan_flag"
                       name="group">是否参与拼团
          </el-checkbox>
        </el-tooltip>
      </el-form-item>
      ...
      <div class="group" style="margin-left: 120px;margin-bottom: 22px;"
           v-if="ruleForm.pintuan_flag">
        <el-form-item label="拼团人数" prop="pintuan_people" style="margin-bottom: 0px">
          <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                      :disabled="!noEditField.pintuan_people">
            <el-input v-model.number="ruleForm.pintuan_people"
                      :disabled="noEditField.pintuan_people" class="sortInput"></el-input>
          </el-tooltip>
        </el-form-item>
        <el-form-item label="拼团价格" prop="pintuan_pricex"
                      style="margin-left: 43px;margin-bottom: 0px">
          <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                      :disabled="!noEditField.pintuan_pricex">
            <el-input v-model="ruleForm.pintuan_pricex" :disabled="noEditField.pintuan_pricex"
                      class="sortInput"></el-input>
          </el-tooltip>
        </el-form-item>
        <el-form-item label="截止时间" prop="pintuan_end_time"
                      style="margin-left: 43px;margin-bottom: 0px">
          <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
      ...
      <el-form-item label="商品利润" prop="Products_Profit">
        <el-input v-model="ruleForm.Products_Profit" :disabled="noEditField.Products_Profit"
                  class="sortInput sortInputs"></el-input>
      ...
      <el-dialog title="预览素材" :visible.sync="preDialogInstance.show">
        <video width="100%" style="max-height: 500px" controls autoplay
               :src="domainFn(preDialogInstance.url)"
               v-if="preDialogInstance.type==='video'"></video>
        <img width="100%" style="max-height: 500px" :src="domainFn(preDialogInstance.url)"
             v-if="preDialogInstance.type==='image'" alt="">
      </el-dialog>
        ...
        <div class="preview-thumb-box preview-box">
          <div class="preview-thumb-item preview-item" v-for="(img,idx) in thumb" :key="idx">
            ...
            <div class="actions thumb-actions">
              ...
              <div class="row">
                <span title="查看图片" class="icon __item-preview" @click="onPreviewFn(img)"><i
                  class="el-icon-zoom-in"></i></span>
                <span title="删除图片" class="icon __item-delete" @click="doRemoveThumb(idx)"><i
                  class="el-icon-delete"></i></span>
              </div>
              <div class="row">
                <span title="向右移动图片" class="icon __item-pre" v-if="idx!=0" @click="prevFn(img,idx)"><i
                  class="el-icon-back"></i></span>
                <span title="向左移动图片" class="icon __item-next" v-if="idx!=(thumb.length-1)"
                      @click="nextFn(img,idx)"><i class="el-icon-right"></i></span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="relative">
            <div class="preview-thumb-box preview-box" v-if="video">
              <div class="preview-thumb-item preview-item">
                ...
                <div class="actions">
                  <span class="__item-preview" @click="onPreviewFn(video,'video')"><i
                    class="el-icon-zoom-in"></i></span>
                  <span class="__item-delete" @click="video=''"><i
                    class="el-icon-delete"></i></span>
                </div>
              </div>
            </div>
          ...
          <div class="margin15-c relative">
            <div class="preview-thumb-box preview-box" v-if="imgs">
              <div class="preview-thumb-item preview-item">
                ...
                <div class="actions">
                  <span class="__item-preview" @click="onPreviewFn(imgs)"><i
                    class="el-icon-zoom-in"></i></span>
      ...
      <el-form-item label="商品简介">
        <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                    :disabled="!noEditField.Products_BriefDescription">
          <el-input maxlength="250" type="textarea"
                    :disabled="noEditField.Products_BriefDescription"
                    v-model="ruleForm.Products_BriefDescription" style="width: 600px;"
                    :rows="4"></el-input>
        </el-tooltip>
      </el-form-item>
      ...
      <el-form-item label="商品类型" prop="Products_Type" title="请输入活动">
        <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
          ...
          <el-select v-model="ruleForm.Products_Type" placeholder="请选择类型"
                     :disabled="noEditField.Products_Type" style="width: 160px">
            ...
            <div class="specs-item-list">
              <div v-for="(val,idx_val) in row.vals" class="spec-item-box">
                <div class="input-wrap"
                     style="width: 110px;margin-left: 10px;display: inline-block;position: relative">
                  <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                              :disabled="!noEditField.Products_Type">
                    <el-autocomplete
                      class="inline-input"
                ...
                <template v-if="idx_row==0&&skuImg">
                  <div class="preview-spec-box preview-box"
                       style="display: block;margin: 10px auto;" v-if="specs[0].imgs[idx_val]">
                    <div class="preview-thumb-item preview-item"
                         style="display: block;margin: 10px auto;">
                      <img class="img" :src="specs[0].imgs[idx_val]"/>
                      <div class="actions">
                        <span class="__item-preview" @click="onPreviewFn(specs[0].imgs[idx_val])"><i
                          class="el-icon-zoom-in"></i></span>
                        <span class="__item-delete" @click="removeSpecPic(idx_val)"><i
                          class="el-icon-delete"></i></span>
                      </div>
                    </div>
                  </div>
                  <div style="display: block;margin: 10px auto;" class="js-finder-label"
                       v-if="!specs[0].imgs[idx_val]" @click="openFinderBySpec(idx_val)">
                    <i class="el-icon-plus"></i>
                  </div>
                </template>
              ...
              <span class="margin15-c" style="cursor: pointer;color: #428CF7"
                    @click="skuAdd(idx_row)">添加规格值</span>
              <el-checkbox style="margin-left: 5px" v-if="idx_row==0" v-model="skuImg" name="sku">
                添加规格图片
              </el-checkbox>
            </div>
          </div>
            ...
            <template v-if="skus.length>1">
              <template v-for="(sku,idx) in skus">
                <tr class="tr">
                  ...
                  <td class="td borderRight inputSku">
                    <el-tooltip class="item" effect="light" :content="textTitle"
                                placement="top-start" :disabled="!noEditField.Products_Type">
                      <el-input :disabled="noEditField.Products_Type" style="width: 160px"
                                v-if="skuList[idx]" v-model="skuList[idx].Attr_Price"/>
                    </el-tooltip>
                  </td>
                  <td class="td borderRight inputSku">
                    <el-tooltip class="item" effect="light" :content="textTitle"
                                placement="top-start" :disabled="!noEditField.Products_Type">
                      <el-input :disabled="noEditField.Products_Type" style="width: 160px"
                                v-if="skuList[idx]" v-model="skuList[idx].Property_count"/>
                    </el-tooltip>
                  </td>
                  <td class="td borderRight inputSku">
                    <el-tooltip class="item" effect="light" :content="textTitle"
                                placement="top-start" :disabled="!noEditField.Products_Type">
                      <el-input :disabled="noEditField.Products_Type" style="width: 160px"
                                v-if="skuList[idx]" v-model="skuList[idx].Supply_Price"/>
                    </el-tooltip>
                  </td>
                  <td class="td borderRight inputSku" v-if="ruleForm.pintuan_flag">
                    <el-tooltip class="item" effect="light" :content="textTitle"
                                placement="top-start" :disabled="!noEditField.Products_Type">
                      <el-input :disabled="noEditField.Products_Type" style="width: 160px"
                                v-if="skuList[idx]" v-model="skuList[idx].pt_pricex"/>
                    </el-tooltip>
                  </td>
                </tr>
              </template>
              ...
              <tr class="tr" v-if="specs.length>1">
                ...
                <td class="td borderRight inputSku">
                  <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                              :disabled="!noEditField.Products_Type">
                    <el-input :disabled="noEditField.Products_Type" style="width: 160px"
                              v-if="skuList[0]" type="number" v-model="skuList[0].Attr_Price"/>
                  </el-tooltip>
                </td>
                <td class="td borderRight inputSku">
                  <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                              :disabled="!noEditField.Products_Type">
                    <el-input :disabled="noEditField.Products_Type" style="width: 160px"
                              v-if="skuList[0]" v-model="skuList[0].Property_count"/>
                  </el-tooltip>
                </td>
                <td class="td borderRight inputSku">
                  <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                              :disabled="!noEditField.Products_Type">
                    <el-input :disabled="noEditField.Products_Type" style="width: 160px"
                              v-if="skuList[0]" v-model="skuList[0].Supply_Price"/>
                  </el-tooltip>
                </td>
                <td class="td borderRight inputSku" v-if="ruleForm.pintuan_flag">
                  <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                              :disabled="!noEditField.Products_Type">
                    <el-input style="width: 160px" :disabled="noEditField.Products_Type"
                              v-if="skuList[0]" v-model="skuList[0].pt_pricex"/>
                  </el-tooltip>
                </td>
              </tr>
                ...
                <tr class="tr" v-for="(item,idx) of skuList">
                  ...
                  <td class="td borderRight inputSku">
                    <el-tooltip class="item" effect="light" :content="textTitle"
                                placement="top-start" :disabled="!noEditField.Products_Type">
                      <el-input :disabled="noEditField.Products_Type" style="width: 160px"
                                type="number" v-model="skuList[idx].Attr_Price"/>
                    </el-tooltip>
                  </td>
                  <td class="td borderRight inputSku">
                    <el-tooltip class="item" effect="light" :content="textTitle"
                                placement="top-start" :disabled="!noEditField.Products_Type">
                      <el-input :disabled="noEditField.Products_Type" style="width: 160px"
                                v-model="skuList[idx].Property_count"/>
                    </el-tooltip>
                  </td>
                  <td class="td borderRight inputSku">
                    <el-tooltip class="item" effect="light" :content="textTitle"
                                placement="top-start" :disabled="!noEditField.Products_Type">
                      <el-input :disabled="noEditField.Products_Type" style="width: 160px"
                                v-model="skuList[idx].Supply_Price"/>
                    </el-tooltip>
                  </td>
                  <td class="td borderRight inputSku" v-if="ruleForm.pintuan_flag">
                    <el-tooltip class="item" effect="light" :content="textTitle"
            ...
            <tr class="tr">
              <td class="td divTd" colspan="9">
                ...
                <template v-if="allPrice">
                  <span class="span" @click="changePrice('price')">价格</span><span class="span"
                                                                                  @click="changePrice('count')">库存</span><span
                  class="span" @click="changePrice('supply')">成本价</span><span
                  v-if="ruleForm.pintuan_flag" class="span"
                  @click="changePrice('pintuan')">拼团价</span>
                </template>
                <template v-else="!allPrice">
                  <span><el-input v-model="allValue" style="width: 100px;"/><span class="spans"
                                                                                  @click="saveAll">保存</span><span
                    class="spans" @click="delAll">取消</span></span>
                </template>
              </td>
            </tr>
          </table>
      ...
      <el-form-item label="商品承诺">
        <div class="input-wrap" style="width: 160px;display: inline-block;position: relative"
             :style="{marginLeft:index==0?'':'10px'}" v-for="(item,index) of Products_Promise"
             :key="index">
          <el-input v-model="Products_Promise[index]" :disabled="noEditField.Products_Promise"
                    @focus="focusCommit(index)"/>
        ...
        <span class="margin15-c" style="cursor: pointer;color: #428CF7"
              @click="committedAdd">添加承诺</span>
      </el-form-item>

      <el-form-item label="商品重量" prop="Products_Weight">
        <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                    :disabled="!noEditField.Products_Weight">
          <el-input v-model="ruleForm.Products_Weight" :disabled="noEditField.Products_Weight"
                    class="sortInput"></el-input>
          kg
        </el-tooltip>
      </el-form-item>
      ...
      <el-form-item label="运费计算" prop="goods">
        <el-radio-group v-model="ruleForm.goods">
          ...
          <!--          </el-radio>-->
          <el-radio label="1"
                    style="display: block;margin-bottom: 15px;height: 30px;line-height: 30px">
            固定运费
            <template v-if="ruleForm.goods==1">
              <el-input v-model="fix_fee" class="sortInput" placeholder="运费金额：¥"
                        style="width: 160px;margin-left: 23px;"></el-input>
              元
            </template>
          </el-radio>
          <el-radio label="2"
            ...
            <template v-if="ruleForm.goods==2&&yunfei.length>0">
              <el-select v-model="shipping_temp" placeholder="请选择物流模板"
                         style="width: 160px;margin-left: 23px;">
      ...
      <el-form-item label="其他属性" prop="otherAttributes">
        <el-checkbox-group v-model="ruleForm.otherAttributes">
          <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                      :disabled="!noEditField.Products_SoldOut">
            <el-checkbox label="下架" name="otherAttributes"
                         :disabled="noEditField.Products_SoldOut"></el-checkbox>
          </el-tooltip>
          <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                      :disabled="!noEditField.Products_IsNew">
            <el-checkbox label="新品" name="otherAttributes"
                         :disabled="noEditField.Products_IsNew"></el-checkbox>
          </el-tooltip>
          <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                      :disabled="!noEditField.Products_IsHot">
            <el-checkbox label="热卖" name="otherAttributes"
                         :disabled="noEditField.Products_IsHot"></el-checkbox>
          </el-tooltip>
          <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                      :disabled="!noEditField.Products_IsRecommend">
            <el-checkbox label="推荐" name="otherAttributes"
                         :disabled="noEditField.Products_IsRecommend"></el-checkbox>
          </el-tooltip>
        </el-checkbox-group>
      </el-form-item>
      ...
      <el-form-item label="特殊属性">
        <el-checkbox-group v-if="prodConfig.Payment_RmainderEnabled==1"
                           :disabled="noEditField.Products_IsPaysBalance"
                           v-model="ruleForm.Products_IsPaysBalance">
          <el-tooltip class="item" effect="light" :content="textTitle" placement="top-start"
                      :disabled="!noEditField.Products_IsPaysBalance">
        ...
        <el-checkbox-group v-model="ruleForm.prod_limit" style="display: flex">
          ...
          <el-input-number v-model="vipNum" :min="1" label="件" sizi="mini"
                           style="width: 120px;margin-left: 10px"></el-input-number>
      ...
      <el-form-item label="订单类型" prop="orderType">
        <el-radio-group v-model="ruleForm.orderType" :disabled="noEditField.prod_order_type"
                        @change="changeRadio">
          <el-radio label="0" style="display: block;margin-bottom: 15px">实物订单 <span class="font12">( 买家下单 -> 买家付款 -> 商家发货 -> 买家收货 -> 订单完成 )</span>
          </el-radio>
          <el-radio label="1" style="display: block;margin-bottom: 15px">虚拟订单 <span class="font12">( 买家下单 -> 买家付款 -> 系统发送消费券码到买家手机 -> 商家认证消费 -> 订单完成 )</span>
          </el-radio>
          <el-radio label="2" style="display: block;margin-bottom: 15px"><span>其他  <span
            class="font12">( 买家下单 -> 买家付款 -> 订单完成 ) <el-button type="primary" @click="clickRadio"
                                                               v-if="!noEditField.prod_order_type"
                                                               sizi="mini">设置</el-button></span></span>
          </el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item label="商品库存" prop="Products_Count">
        <el-input v-model="ruleForm.Products_Count" :disabled="noEditField.Products_Count"
                  class="sortInput"></el-input>
        <span class="sortMsg">注:若不限则填写10000</span>
      </el-form-item>
      <el-form-item label="退货损坏说明">
        <el-select v-model="ruleForm.refund" placeholder="请选择"
                   :disabled="noEditField.Product_backup" style="width: 160px">
      ...
      <el-form-item label="商品详情">
        <div style="padding-right: 200px" v-if="showEditor">
    ...
    <div class="bottomFixed">
      <el-button size="small" type="primary" :loading="isLoading" @click="submitForm('ruleForm')">
        提交保存
      </el-button>
    ...
    >
      ...
      <el-form :model="ruleForm" :rules="rules" size="small" ref="ruleForm" label-width="0px"
               class="ruleForm">
        ...
        <div class="commissionDiv">
          ...
          <div class="rightTitle">
            <el-form-item label="" prop="sort" style="margin-bottom: 0px;">
              <el-input v-model="area_Proxy_Reward"
                        style="width: 80px;margin-left: 19px;"></el-input>
        ...
        <div class="commissionDiv">
          ...
          <div class="rightTitle">
            <el-form-item label="" prop="sort" style="margin-bottom: 0px;">
              <el-input v-model="commission_ratio"
                        style="width: 80px;margin-left: 19px;"></el-input>
        ...
        <div class="commissionDiv">
          ...
          <div class="rightTitle">
            <el-form-item label="" prop="sort" style="margin-bottom: 0px;margin-left: 19px">
              <el-radio-group style="display: flex;align-items: center;padding-top: 20px;"
                              v-model="manage_commi">
            ...
            <div class="fenxiaoshang" v-for="(fen,fenIndex) of dis_level_list" :key="fenIndex">
              ...
              <div class="paddingAll" v-if="distriboutor_config[fenIndex]">
                <el-form-item label="" prop="sort" class="padding15-t marginBootom"
                              v-for="(dis,disIndex) of Dis_Level_arr" :key="disIndex">
    ...
    >
      <div class="cardTitle">
        <div class="cardTitle" style="margin-right: 10px">
          卡号：
          <el-input v-model="CardIdSelect" class="sortInput" style="width: 100px"></el-input>
        </div>
        <div class="cardTitle" style="margin-right: 10px">
          栏目：
...
<script lang="ts">
  ...
  @Component({
    ...
    beforeRouteLeave(to, form, next) {
      ...
      this.$refs.richtext.destroyTinymce()
      next()
    }
  })
  export default class AddProduct extends Vue {
    ...
    openFinderByVideoCover() {
      FUNFinder.open({
        options: {limit: 1, allow: ['image']},
        callFn: {choose: this.upImgsSuccessCall}
      });
    }
    ...
    openFinderByVideo() {
      FUNFinder.open({
        options: {limit: 1, allow: ['media']},
        callFn: {chooseMedia: this.upVideoSuccessCall}
      });
    }
    ...
    openFinderByThumb() {
      ...
      FUNFinder.open({
        options: {limit: 5 - this.thumb.length, allow: ['image']},
        callFn: {choose: this.upThumbSuccessCall}
      });
    }
    ...
    openFinderBySpec(idx) {
      ...
      FUNFinder.open({
        options: {limit: 1, allow: ['image']},
        callFn: {choose: this.upSpecPicSuccessCall}
      });
    }
    ...
    async created() {
      ...
      await systemProdConfig().then(res => {
        ...
        this.prodConfig.prod_type_list.unshift({
          Attr_ID: 0,
          Attr_Name: [],
          Attr_Values: '',
          Type_ID: 0,
